...
func TestDoStructsMatchCmdspec(t *testing.T) {
	...
	readFields := map[string]bool{
		...
		"skeleton": true, "lines": true, "depth": true, "start_line": true, "end_line": true, "symbols": true, "full": true, "expand": true,
	}
...
func TestAnalyzePatterns_SequentialReads(t *testing.T) {
	sess := session.New()
	// Simulate 4 sequential reads
	sess.RecordOp("read", "cmd/root.go", "", "read_symbol", "file_read", true)
	sess.RecordOp("read", "cmd/batch.go", "handleDo", "read_symbol", "symbol_read", true)
	sess.RecordOp("read", "cmd/commands.go", "", "read_symbol", "file_read", true)
	sess.RecordOp("read", "internal/dispatch/dispatch.go", "Dispatch", "read_symbol", "symbol_read", true)

	suggestions := analyzePatterns(sess)
	found := false
	for _, s := range suggestions {
		if strings.Contains(s, "sequential reads") && strings.Contains(s, "-r cmd/root.go") && strings.Contains(s, "-r cmd/batch.go:handleDo") {
			found = true
		}
	}
	if !found {
		t.Errorf("expected concrete batch command with actual files, got: %v", suggestions)
	}
}

func TestAnalyzePatterns_EditThenRead(t *testing.T) {
	sess := session.New()
	// Simulate edit→read on same file, twice
	sess.RecordOp("edit", "cmd/root.go", "Execute", "replace_text", "text_replaced", true)
	sess.RecordOp("read", "cmd/root.go", "Execute", "read_symbol", "symbol_read", true)
	sess.RecordOp("edit", "cmd/batch.go", "handleDo", "replace_text", "text_replaced", true)
	sess.RecordOp("read", "cmd/batch.go", "handleDo", "read_symbol", "symbol_read", true)

	suggestions := analyzePatterns(sess)
	found := false
	for _, s := range suggestions {
		if strings.Contains(s, "--read-back") {
			found = true
		}
	}
	if !found {
		t.Errorf("expected --read-back suggestion, got: %v", suggestions)
	}
}

func TestAnalyzePatterns_NoFalsePositive(t *testing.T) {
	sess := session.New()
	// Two reads with a search in between — not sequential
	sess.RecordOp("read", "cmd/root.go", "", "read_symbol", "file_read", true)
	sess.RecordOp("search", "", "", "search", "search", true)
	sess.RecordOp("read", "cmd/batch.go", "", "read_symbol", "file_read", true)

	suggestions := analyzePatterns(sess)
	for _, s := range suggestions {
		if strings.Contains(s, "sequential reads") {
			t.Errorf("should not suggest batching non-sequential reads, got: %v", suggestions)
		}
	}
}
