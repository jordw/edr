...
// TestDogfood_RealGoFiles parses real Go files under a directory set
// via EDR_SCOPE_GO_DOGFOOD_DIR and reports resolution stats. Enforces:
// no panics, every unresolved ref has a reason code, no scope=0 refs
// (stack underflow).
func TestDogfood_RealGoFiles(t *testing.T) {
	dir := os.Getenv("EDR_SCOPE_GO_DOGFOOD_DIR")
	if dir == "" {
		t.Skip("EDR_SCOPE_GO_DOGFOOD_DIR not set")
	}

	var files []string
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		if err != nil || info.IsDir() {
			if info != nil && info.IsDir() && (info.Name() == ".git" || info.Name() == ".edr" || info.Name() == "testdata") {
	...
	if totalRefs > 0 {
		...
		t.Logf("unresolved external:%d (%.1f%%) (name NOT in file — import/etc)",
