...
// TestDogfood_RealPHPFiles parses real PHP files under a directory set
// via EDR_SCOPE_PHP_DOGFOOD_DIR. Reports resolution stats. Enforces:
// no panics, no scope=0 refs, every unresolved ref has a reason code.
func TestDogfood_RealPHPFiles(t *testing.T) {
	dir := os.Getenv("EDR_SCOPE_PHP_DOGFOOD_DIR")
	if dir == "" {
		t.Skip("EDR_SCOPE_PHP_DOGFOOD_DIR not set")
	}
	...
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		if info.IsDir() {
			n := info.Name()
			if n == ".git" || n == ".edr" || n == "vendor" || n == "node_modules" {
				return filepath.SkipDir
			}
			return nil
		}
		if strings.HasSuffix(path, ".php") && !strings.HasSuffix(path, ".test.php") {
