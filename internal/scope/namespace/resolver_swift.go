...
import (
	...
	scopeswift "github.com/jordw/edr/internal/scope/swift"
)

// SwiftResolver is a Resolver for Swift codebases. v1 only
...
func NewSwiftResolver(repoRoot string) *SwiftResolver {
...
func (r *SwiftResolver) Source(file string) []byte {
	r.parseMu.Lock()
...
func (r *SwiftResolver) CanonicalPath(file string) string {
	return swiftCanonicalPathForFile(file, r.repoRoot)
}

func (r *SwiftResolver) Result(file string) *scope.Result {
	if !strings.EqualFold(filepath.Ext(file), ".swift") {
		return nil
	}
	r.parseMu.Lock()
	...
	res := scopeswift.ParseCanonical(file, canonical, src)
	r.parseMu.Lock()
...
func (r *SwiftResolver) FilesForImport(importSpec, importingFile string) []string {
	return nil
}

// SamePackageFiles returns siblings in the same directory.
func (r *SwiftResolver) SamePackageFiles(importingFile string) []string {
