...
// TestDogfood_RealCFiles parses real C files under a directory set via
// EDR_SCOPE_C_DOGFOOD_DIR and reports resolution stats. Enforces:
// no panics, every unresolved ref has a reason code, no scope=0 refs
// (stack underflow).
func TestDogfood_RealCFiles(t *testing.T) {
	dir := os.Getenv("EDR_SCOPE_C_DOGFOOD_DIR")
	if dir == "" {
		t.Skip("EDR_SCOPE_C_DOGFOOD_DIR not set")
	}
	...
	if totalRefs > 0 {
		...
		t.Logf("unresolved external:%d (%.1f%%) (name NOT in file — include/macro/etc)",
