...
func TestMerge(t *testing.T) {
	a := &IndexData{
		Header: Header{NumFiles: 2, NumTrigrams: 0, GitMtime: 100},
		Files: []FileEntry{
			{Path: "a.go", Mtime: 100, Size: 10},
			{Path: "b.go", Mtime: 100, Size: 20},
		},
	}
	b := &IndexData{
		Header: Header{NumFiles: 2, NumTrigrams: 0, GitMtime: 200},
		Files: []FileEntry{
			{Path: "b.go", Mtime: 200, Size: 25}, // newer
			{Path: "c.go", Mtime: 200, Size: 30},
		},
	}

	// Build some posting data for both
	aMap := map[Trigram][]uint32{{'x', 'y', 'z'}: {0, 1}}
	a.Postings, a.Trigrams = BuildPostings(aMap)
	a.Header.NumTrigrams = uint32(len(a.Trigrams))

	bMap := map[Trigram][]uint32{{'x', 'y', 'z'}: {0, 1}}
	b.Postings, b.Trigrams = BuildPostings(bMap)
	b.Header.NumTrigrams = uint32(len(b.Trigrams))

	merged := Merge(a, b)

	if merged.Header.NumFiles != 3 {
		t.Errorf("merged files: got %d, want 3", merged.Header.NumFiles)
	}
	if merged.Header.GitMtime != 200 {
		t.Errorf("merged git mtime: got %d, want 200", merged.Header.GitMtime)
	}

	// b.go should have the newer mtime
	for _, f := range merged.Files {
		if f.Path == "b.go" {
			if f.Mtime != 200 {
				t.Errorf("b.go mtime: got %d, want 200", f.Mtime)
			}
			if f.Size != 25 {
				t.Errorf("b.go size: got %d, want 25", f.Size)
			}
		}
	}
}
...
func TestIncrementalBuild(t *testing.T) {
	...
	n, err := BuildIncremental(dir, edrDir, []string{
	...
	if len(jnls) != 1 {
		t.Errorf("expected 1 journal, got %d", len(jnls))
	}

	// Second incremental build (same PID, should merge)
	n, err = BuildIncremental(dir, edrDir, []string{
	...
	// Still one journal (merged)
	jnls, _ = filepath.Glob(filepath.Join(edrDir, JournalPfx+"*"+JournalSfx))
	if len(jnls) != 1 {
		t.Errorf("expected 1 journal after merge, got %d", len(jnls))
	}

	// Query should find both files
	tris := QueryTrigrams("package")
	candidates, ok := Query(edrDir, tris)
	if !ok {
		t.Fatal("Query returned not-ok")
	}
...
func TestCompact(t *testing.T) {
	...
	// Create main index + a journal
	writeFile(t, dir, "a.go", "package a\nfunc alpha() {}")
	writeFile(t, dir, "b.go", "package b\nfunc beta() {}")

	d := BuildFull(dir, []string{filepath.Join(dir, "a.go")}, 100)
	os.WriteFile(filepath.Join(edrDir, MainFile), d.Marshal(), 0600)

	d2 := BuildFull(dir, []string{filepath.Join(dir, "b.go")}, 200)
	os.WriteFile(filepath.Join(edrDir, JournalPfx+"12345"+JournalSfx), d2.Marshal(), 0600)

	Compact(edrDir)

	// Main should exist, journals should be gone
	if _, err := os.Stat(filepath.Join(edrDir, MainFile)); err != nil {
		t.Error("main index missing after compact")
	}
	jnls, _ := filepath.Glob(filepath.Join(edrDir, JournalPfx+"*"+JournalSfx))
	if len(jnls) != 0 {
		t.Errorf("expected 0 journals after compact, got %d", len(jnls))
	}
