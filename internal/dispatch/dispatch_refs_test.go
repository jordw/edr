...
// TestRename_GoStdlibInterfaceConformanceRefuses guards hard-blocker 2:
// renaming a method whose name + signature satisfies a stdlib interface
// (here io.Reader.Read) silently breaks compile because the interface
// declaration lives in the stdlib and can't be rewritten. The dispatch
// layer must refuse with a structured result naming the interface.
func TestRename_GoStdlibInterfaceConformanceRefuses(t *testing.T) {
	db, dir := setupRefsRepo(t, map[string]string{
		"go.mod": "module example.com/foo\n",
		"a.go": `package foo
...
func (r *LineReader) Read(p []byte) (int, error) { return 0, io.EOF }
`,
	})
	res, err := dispatch.Dispatch(context.Background(), db, "rename",
		[]string{"a.go:Read"},
		map[string]any{"new_name": "ReadLine", "cross_file": true})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
	...
	if rr.Status != "refused" {
		t.Errorf("expected Status=refused, got %q", rr.Status)
	}
	if len(rr.Warnings) == 0 || !strings.Contains(rr.Warnings[0], "io.Reader") {
		t.Errorf("expected warning naming io.Reader, got %v", rr.Warnings)
	}
	// The file must NOT have been touched.
	aData, _ := os.ReadFile(filepath.Join(dir, "a.go"))
	if !strings.Contains(string(aData), "func (r *LineReader) Read") {
		t.Errorf("file was rewritten despite refusal; got:\n%s", aData)
	}
}

// TestRename_GoStdlibInterfaceMissingImportSucceeds verifies the
// import-presence guard: a Read method on a type whose file does NOT
// import "io" should NOT trip the conformance gate. Without an io
// import, no caller in this file uses our type as io.Reader, and the
// duck-typing risk is judged acceptable here. Keeps the gate from
// over-refusing on incidental Read methods (e.g. a config parser).
func TestRename_GoStdlibInterfaceMissingImportSucceeds(t *testing.T) {
	db, _ := setupRefsRepo(t, map[string]string{
		"go.mod": "module example.com/foo\n",
		"a.go": `package foo
...
func (r *ConfigReader) Read(p []byte) (int, error) { return 0, nil }
`,
	})
	res, err := dispatch.Dispatch(context.Background(), db, "rename",
		[]string{"a.go:Read"},
		map[string]any{"new_name": "ReadConfig", "cross_file": true})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
	rr, ok := res.(*output.RenameResult)
	...
	if rr.Status == "refused" {
		t.Errorf("rename was refused but no io import is present; warnings=%v", rr.Warnings)
	}
}
