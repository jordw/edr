...
edr is both the tool and the target. If you break the build, you lose your tools. Use `edr checkpoint` before risky changes. If a broken edit prevents `go build`, fall back to standard shell tools (`cat`, `sed`) to fix the compile error, then rebuild.
...
**internal/setup/**: `edr setup` installer. Injects agent instructions into global configs (~/.claude, .cursorrules, etc.). Instructions are in `instructions/*.md`, token-capped at 500.
