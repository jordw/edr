...
**cmd/**: Cobra command wiring. `commands.go` has standalone commands, `batch.go`/`batch_cmd.go` handle batch mode (`-r`, `-s`, `-e`, `-w`). `commands.go` also builds `edr next` results (`buildNextResult`, `computeCurrentItems`, `computeFixItems`).
