...
**internal/index/**: Pure-Go symbol extraction via lexkit-based parsers, on-demand symbol store, signature extraction. `store.go` defines the `SymbolStore` interface; `ondemand.go` implements it by parsing files on demand. `langs.go:Parse` dispatches by extension to per-language `parse_<lang>.go` modules (built on `internal/lexkit`). `signatures.go` extracts one-line signatures per language. `types.go` defines `SymbolInfo`.
...
1. Write `internal/index/parse_<lang>.go` using `internal/lexkit` primitives
2. Add a `*ToSymbolInfo` adapter and wire it into the dispatch switch in `internal/index/langs.go:Parse`
3. Add a signature extractor in `internal/index/signatures.go` (often the brace-style helper suffices)
4. Add a fixture test in `internal/index/parse_<lang>_test.go` and a ground-truth case in one of the `parse_groundtruth*_test.go` files
