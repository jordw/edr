...
Flags: `--text`, `--include`, `--exclude`, `--context`, `--limit`, `--budget`, `--full`
...
Flags: `--dir`, `--glob`, `--type`, `--grep`, `--budget`, `--full`
...
| `-s "pattern"` | Search | `--text`, `--include`, `--exclude`, `--context N`, `--limit N`, `--full` |
...
- **Default budget**: search and map cap output at 2000 tokens by default (use `--budget N` to override, `--full` to uncap)
